...
ControlNode::ControlNode()
{
	...
	parameter_MaxControl = 5.0;
	parameter_InitialReachDist = 0.1;
	parameter_StayWithinDist = 0.15;
	parameter_StayTime = 0.8;
	isControlling = false;
	currentKI = NULL;
...
void ControlNode::droneposeCb(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	//if reached tell patternRec do next
...
void ControlNode::popNextCommand(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	while(currentKI == NULL && commandQueue.size() > 0)
	{
		...
		//start Pattern Recognition for eraser
		else if(command=="startPR")			//!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
			isPR=true;
			ROS_INFO("forstart: x=%f y=%f z=%f yaw=%f",statePtr->x,statePtr->y,statePtr->z,statePtr->yaw);
			currentKI=new KIFlyTo(
					DronePosition(TooN::makeVector(statePtr->x,statePtr->y,statePtr->z+0.5)+parameter_referenceZero.pos,	//go up higher than 1m
						0.0+parameter_referenceZero.yaw),//statePtr->yaw+parameter_referenceZero.yaw),
					parameter_StayTime,
		...
		//stop Pattern Recognition for eraser
		else if(command=="stopPR")			//!!!!!!!!!!!!!!!!!!!!!!!!!
		{
			isPRing=false;
			isPR=false;
			publishCommand("prh SystemStop");
		}

		// goto
		else if(sscanf(command.c_str(),"goto %f %f %f %f",&parameters[0], &parameters[1], &parameters[2], &parameters[3]) == 4)
		{
			ROS_INFO("currentState: x=%f y=%f z=%f yaw=%f",statePtr->x,statePtr->y,statePtr->z,statePtr->yaw);
			currentKI = new KIFlyTo(
				DronePosition(
...
void ControlNode::comCb(const std_msgs::StringConstPtr str)
{
	...
	if(str->data.length() > 2 && str->data.substr(0,2) == "c ")
	{
		...
		else if(cmd.length() == 5 && cmd.substr(0,5) == "hover")
		{
			sendControlToDrone(hoverCommand);
		}
		else if(cmd.length() == 4 && cmd.substr(0,4) == "turn")
		{
			ControlCommand c;
			c.yaw=-1.5;
...
void ControlNode::updateControl(const tum_ardrone::filter_stateConstPtr statePtr) {
	if (currentKI->update(statePtr) && commandQueue.size() >= 0) {	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
		if(isPR)
		{
			if(!isPRing)		//start or reach
			{
				publishCommand("pr SystemStart");
				ROS_INFO("pr SystemStart");
				isPRing=true;
			}
			else
			{
				publishCommand("pr TargetReached");
				ROS_INFO("pr TargetReached");
			}
			/*isPRing=true;
			publishCommand("pr TargetReached");
			ROS_INFO("pr TargetReached");*/
