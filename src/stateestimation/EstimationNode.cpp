...
EstimationNode::EstimationNode()
{
	...
	vidp_sub		 =nh_.subscribe(video_channel,10,&EstimationNode::vidpCb,this);
	...
	patternRec=new PatternRec(this);		//!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void EstimationNode::vidpCb(const sensor_msgs::ImageConstPtr img)
{
	//give to Rec
	patternRec->currentImage(img);
}
...
void EstimationNode::comCb(const std_msgs::StringConstPtr str)
{
	...
	if(str->data.length() == 14 && str->data.substr(0,3) == "pr ")		//!!!!!!!!!!!!!!1
	{
		patternRec->startSystem();
	}
	/*if(str->data.length() == 9 && str->data.substr(0,4) == "c st")
		patternRec->startSystem();*/
	if(str->data.length() == 16 && str->data.substr(0,4) == "pr T")
	{
		patternRec->prSetReached();
	}
	if(str->data.length() == 14 && str->data.substr(0,4) == "prh ")
	{
		patternRec->stopSystem();
	}
